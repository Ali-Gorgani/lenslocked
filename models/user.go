...
func (service *UserService) Create(email, password string) (*User, error) {
	...
	row := service.DB.QueryRow("INSERT INTO users(email, password_hash) VALUES($1, $2) RETURNING id;", email, passwordHash)
...
func (service *UserService) UpdatePassword(userID int, password string) error {
	...
	passwordHash := string(hashedBytes)
	_, err = service.DB.Exec("UPDATE users SET password_hash = $2 WHERE id = $1;", userID, passwordHash)
	if err != nil {
		return fmt.Errorf("update password: %w", err)
	}
	return nil
